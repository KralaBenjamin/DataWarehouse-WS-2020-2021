...
## Hilfsmethode
def display_semester(mean, median, mode) :
    ...
    plt.text(mean, max_ylim*0.5, 'Mean: {:.2f}'.format(mean),  fontsize=16,rotation=-90)
    plt.text(mode, max_ylim*0.5, 'Mode: {:.2f}'.format(mode),  fontsize=16,rotation=-90)
    plt.text(median, max_ylim*0.5, 'Median: {:.2f}'.format(median),  fontsize=16,rotation=-90)
...
def plotEstimators (estimator, iris) :
    ...
    if estimator is not None:
        plt.scatter(iris['petal_length'], iris['petal_width'], c=estimator.labels_)

    plt.show()

    # Messe Genauigkeit als Anzahl über die falsch geclusterten Punkte
    if estimator is not None:
        r = iris2.join(pd.Series(estimator.labels_, name="KMeans")).groupby(['species', 'KMeans']).size()
